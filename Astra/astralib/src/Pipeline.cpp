...
void Astra::RayTracingPipeline::createPipeline(VkDevice vkdev, const std::vector<VkDescriptorSetLayout>& descsets, nvvk::ResourceAllocatorDma& alloc)
{
	...
	// raygen
	stage.module = nvvk::createShaderModule(vkdev, nvh::loadFile("spv/raytrace_iter.rgen.spv", true, defaultSearchPaths, true));
	stage.stage = VK_SHADER_STAGE_RAYGEN_BIT_KHR;
	stages[eRaygen] = stage;

	// miss
	stage.module = nvvk::createShaderModule(vkdev, nvh::loadFile("spv/raytrace_iter.rmiss.spv", true, defaultSearchPaths, true));
	...
	// chit
	stage.module = nvvk::createShaderModule(vkdev, nvh::loadFile("spv/raytrace_iter.rchit.spv", true, defaultSearchPaths, true));
